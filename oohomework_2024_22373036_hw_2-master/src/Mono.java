...
public class Mono {
    ...
    public String toStringWithoutExp() {
        StringBuilder termBuilder = new StringBuilder();
        termBuilder.append(coefficient).append("*x^").append(powerExponentX)
                .append("y^").append(powerExponentY).append("z^").append(powerExponentZ);
        return termBuilder.toString();
    ...
    static Mono toMono(String term) {
        ...
        for (String factor : Expr.splitFactor(term)) {
            if (factor.startsWith("exp")) {
                ...
                exponent.append("(").append(expContent).append(")").append("*").append(expPower).append("+");
            }
    ...
    public static void simplyExp(StringBuilder termTerm, BigInteger coefficient, StringBuilder exponent) {
        if (!coefficient.equals(BigInteger.ZERO)) {
            if (exponent.length() != 0) {
                ...
                PreProcessr.preProcessor(termTerm);
